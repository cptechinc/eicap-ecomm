<?php
	...
	if (!empty($cart->custid) && $cart->custid != $config->defaultcustid) {
		...
		$page->body = $config->twig->render('customer/search-form.twig', ['page' => $page, 'page_title' => $page->title, 'customersearch' => $customersearch, 'resultscount' => $resultscount, 'custindex' => $custindex, 'input' => $q, 'function' => $input->get->function, 'paginator' => $paginator]);
        include __DIR__ . "/basic-page.php";
	}
?>
